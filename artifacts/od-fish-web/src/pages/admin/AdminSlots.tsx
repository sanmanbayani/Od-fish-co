  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <Button className="w-full md:w-auto" onClick={() => showDialog()}><Plus className="w-4 h-4 mr-2" /> Add Slot</Button>
      </div>
                  ...
                  {slots.map(slot => {
                    ...
                    return (
                      <TableRow key={slot.id}>
                        <TableCell>
                          <div className="font-bold text-primary">{slot.label}</div>
                        ...
                        <TableCell>
                          <div className="w-52 max-w-full space-y-1.5">
                            <div className="flex justify-between text-xs">
                              <span className={full ? "text-destructive font-medium" : ""}>{slot.ordersToday} / {slot.capacity} orders</span>
                              <span>{percent}%</span>
                            </div>
                            <div className="h-2 rounded-full bg-muted overflow-hidden">
                              <div className={`h-full rounded-full ${full ? "bg-destructive" : percent >= 75 ? "bg-orange-500" : "bg-primary"}`} style={{ width: `${percent}%` }} />
                            </div>
                          </div>
                        </TableCell>
                        <TableCell>
                          <div className="flex items-center gap-2">
                            <Switch checked={slot.isOpen} disabled={setSlotOpen.isPending} onCheckedChange={value => toggle(slot, value)} />
                            <Badge variant="outline" className={slot.isOpen ? "bg-green-100 text-green-800 border-green-200" : "bg-muted text-muted-foreground"}>{slot.isOpen ? "Open" : "Closed"}</Badge>
                          </div>
                        </TableCell>
                        <TableCell className="text-right">
                          <Button variant="ghost" size="sm" onClick={() => showDialog(slot)}>Edit</Button>
                        </TableCell>
                      </TableRow>
                    );
                  })}
            ...
            <DataList>
              {slots.map(slot => {
                ...
                return (
                  <DataListItem
                    key={slot.id}
                    title={<span className="font-bold text-primary">{slot.label}</span>}
                    subtitle={<span className="font-mono">{slot.startTime}–{slot.endTime}</span>}
                    trailing={
                      <Badge variant="outline" className={slot.isOpen ? "bg-green-100 text-green-800 border-green-200" : "bg-muted text-muted-foreground"}>
                        {slot.isOpen ? "Open" : "Closed"}
                      </Badge>
                    }
                    actions={
                      <>
                        <Button variant="outline" className="h-11" onClick={() => showDialog(slot)}>Edit</Button>
                        <Button variant="outline" className="h-11" disabled={setSlotOpen.isPending} onClick={() => toggle(slot, !slot.isOpen)}>
                          {slot.isOpen ? "Close slot" : "Open slot"}
                        </Button>
                      </>
                    }
                  >
                    <DataListField label="Cutoff">
                      <span className="font-mono text-sm">{slot.cutoffTime}</span>
                    </DataListField>
                    <div className="w-full space-y-1.5 pt-1.5">
                      <div className="flex justify-between text-xs w-full text-muted-foreground">
                        <span className={full ? "text-destructive font-medium" : ""}>
                          Today's Load: {slot.ordersToday} / {slot.capacity} orders
                        </span>
                        <span>{percent}%</span>
                      </div>
      ...
      <Dialog open={open} onOpenChange={setOpen}><DialogContent>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="sm:col-span-2 space-y-1.5"><Label>Customer-facing label</Label><Input value={form.label} onChange={e => setForm({ ...form, label: e.target.value })} placeholder="7 AM - 10 AM" /></div>
          ...
          <div className="sm:col-span-2 flex items-center gap-3"><Switch checked={form.isOpen} onCheckedChange={isOpen => setForm({ ...form, isOpen })} /><Label>Open for customer orders</Label></div>
        </div>
